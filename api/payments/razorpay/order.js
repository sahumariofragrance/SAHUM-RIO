...
function setSecurityHeaders(res) { res.setHeader("Content-Type", "application/json"); res.setHeader("Cache-Control", "no-store"); res.setHeader("X-Content-Type-Options", "nosniff"); res.setHeader("X-Frame-Options", "DENY"); }
...
module.exports = async (req, res) => {
  ...
  const frontend = Number(frontendAmount); const mismatch = Number.isInteger(frontend) && frontend !== amount;
  ...
  try {
    ...
    const order = await razorpay.orders.create({ amount, currency: "INR", receipt: `rcpt_${crypto.randomBytes(8).toString("hex")}`, notes: {
      customer_name: sanitizeText(safeCustomer.name), customer_phone: sanitizeText(safeCustomer.phone, 20), customer_email: sanitizeText(safeCustomer.email, 200), server_verified_amount_inr: String(amountINR), total_qty: String(count),
    }});
    ...
    return res.status(502).json({ message: "Unable to create payment order. Please try again." });
  }
};
