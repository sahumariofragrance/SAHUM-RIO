...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (eventName === "payment.captured") {
      ...
      if (!Number.isSafeInteger(amount) || amount <= 0 || currency !== "INR" || payment.status !== "captured") {
        return res.status(400).json({ message: "Captured payment payload is invalid" });
      }
      ...
      return res.status(200).json({ received: true, finalized: Boolean(order?.id) });
    }
