...
export default function HomePage({ onProductNavigate, setCurrentPage }) {
  ...
  return <>
    ...
    {editorialProduct && <section className="grid border-y border-[var(--color-border)] lg:grid-cols-2">
      <button type="button" onClick={() => onProductNavigate?.(editorialProduct)} className="group relative min-h-[520px] overflow-hidden bg-[var(--color-surface-muted)]" aria-label={"View " + editorialProduct.name}>
        <img src={editorialProduct.image} alt={editorialProduct.alt || editorialProduct.name} className="absolute inset-0 h-full w-full object-cover transition duration-[1400ms] ease-out group-hover:scale-[1.015]" />
      </button>
      <div className="flex min-h-[520px] items-center bg-[#171614] px-6 py-14 text-[#f7f3eb] sm:px-10 md:px-16 lg:px-[7vw]"><div className="max-w-xl">
        <p className="text-[9px] font-semibold uppercase tracking-[0.22em] text-white/45">The house</p>
        <h2 className="mt-7 font-serif text-[clamp(3.4rem,5.7vw,6.5rem)] font-normal leading-[0.9] tracking-[-0.035em]">Space for the<span className="block italic">fragrance itself.</span></h2>
        <p className="mt-8 max-w-md text-sm leading-7 text-white/60">We keep the presentation quiet so each perfume can be discovered on its own terms.</p>
        <button onClick={() => setCurrentPage?.("about")} className="group mt-8 inline-flex items-center gap-3 border-b border-white/65 pb-1 text-[10px] font-semibold uppercase tracking-[0.16em]">About SAHUMäRIO<ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" /></button>
      </div></div>
    </section>}
