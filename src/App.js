...
import { NOT_FOUND_META, PAGE_META, collectionJsonLd, productMeta } from "./seo/site";
import { applyPageMeta, setPageJsonLd } from "./seo/head";
...
export default function App(){const{user,loading:authLoading,isGuest}=useAuth();const{bySlug,products,loading:productsLoading}=useProducts();const getRouteFromPath=useCallback((pathname="/")=>{const p=pathname==="/"?"/":pathname.replace(/\/+$/,"");const productMatch=p.match(/^\/product\/([a-z0-9-]+)$/);if(productMatch)return{page:"product",slug:productMatch[1]};if(PAGE_PATH_ALIASES[p])return{page:PAGE_PATH_ALIASES[p],slug:null};return{page:Object.entries(PAGE_PATH_MAP).find(([,path])=>path===p)?.[0]||"home",slug:null};},[]);const[route,setRoute]=useState(()=>getRouteFromPath(window.location.pathname));const[isMenuOpen,setIsMenuOpen]=useState(false);const currentPage=route.page;
useEffect(()=>{
  if(currentPage==="product"){
    const product=bySlug.get(route.slug);
    // Product JSON-LD (with review ratings) is owned by ProductPage.
    if(product)applyPageMeta(productMeta(product));
    else if(!productsLoading){applyPageMeta({...NOT_FOUND_META,path:`/product/${route.slug}`});setPageJsonLd(null);}
    return;
  }
  const meta=PAGE_META[currentPage]||PAGE_META.home;
  applyPageMeta(meta);
  setPageJsonLd(currentPage==="perfumes"&&products.length?collectionJsonLd(products):null);
},[currentPage,route.slug,bySlug,products,productsLoading]);
